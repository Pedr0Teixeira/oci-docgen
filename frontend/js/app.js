...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /**
   * Shows the progress indicator overlay.
   * @param {boolean} isSimulated - If true, hides the spinner for simulated percentage-based progress.
   */
  ...
  /**
   * Shows or hides the generic loading spinner overlay.
  ...
  function updateUiForDocType() {
    ...
    let buttonText = 'Buscar Dados da(s) Instância(s)';
    if (selectedDocType === 'full_infra') {
      buttonText = 'Buscar Dados da Infraestrutura';
    } else if (selectedDocType === 'kubernetes') {
   ...
   * @param {HTMLElement} except - An optional element to exclude from closing.
   */
  ...
  const populateDocTypes = () => {
    const docTypes = [
      { id: 'new_host', name: 'Documentação de Novo Host' },
      { id: 'full_infra', name: 'Documentação de Infraestrutura' },
      { id: 'kubernetes', name: 'Documentação de Kubernetes' }
    ];
  ...
  /**
   * Orchestrator function to fetch details based on the selected document type.
   */
  ...
  /**
   * Fetches comprehensive details for all resources in a compartment.
   */
  const fetchAllInfrastructureDetails = async () => {
    ...
    const steps = (selectedDocType === 'kubernetes') ?
      [
        { pct: 25, msg: 'Buscando clusters OKE...' },
        { pct: 50, msg: 'Analisando Node Pools...' },
        { pct: 75, msg: 'Verificando conectividade de rede...' }
      ] :
      [
        ...
        { pct: 75, msg: 'Inspecionando Load Balancers e OKE...' },
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/${selectedRegion}/infrastructure-details/${selectedCompartmentId}`, {
        method: 'POST'
      });
      if (!response.ok) {
      ...
      summaryContainer.innerHTML = generateInfrastructureSummary(data);
  ...
  /**
   * Fetches details for a specific list of selected instances.
   */
  const fetchAllInstanceDetails = async () => {
    ...
    showProgress(false);
    updateProgress(0, `Coletando 0 de ${instanceIds.length} instâncias...`);
    detailsContainer.classList.add('hidden');
    allInfrastructureData = {
      instances: [],
      vcns: [],
    ...
    const progressInterval = setInterval(() => {
      if (simulatedProgress < 90) {
        simulatedProgress += 2;
        const instancesProcessed = Math.min(instanceIds.length, Math.floor((simulatedProgress / 90) * instanceIds.length));
        updateProgress(simulatedProgress, `Coletando ${instancesProcessed} de ${instanceIds.length} instâncias...`);
      }
    }, 150);
    ...
    try {
      ...
      summaryContainer.innerHTML = generateInfrastructureSummary(data, true);
      detailsContainer.classList.remove('hidden');
      updateProgress(100, 'Dados coletados com sucesso!');
   ...
   * @param {boolean} isNewHostFlow - True if the data is for the 'New Host' flow.
  ...
  function generateInfrastructureSummary(data, isNewHostFlow = false) {
    ...
    const volumeGroupsHtml = volume_groups?.length > 0 ? volume_groups.map(vg => {
      ...
      const backupHtml = validation.has_backup_policy ? `<span class="validation-ok"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg> ${validation.policy_name}</span>` : `<span class="validation-fail"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg> Nenhuma</span>`;
      const crossRegionHtml = validation.is_cross_region_replication_enabled ? `<span class="validation-ok"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg> Habilitada</span>` : `<span class="validation-fail"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg> Desabilitada</span>`;
      const membersListHtml = vg.members && vg.members.length > 0 ? `<ul class="member-list">${vg.members.map(m => `<li>${m}</li>`).join('')}</ul>` : `<p class="no-data-message">Nenhum membro encontrado.</p>`;
      const cardContent = `<div class="content-block"><h5 class="subheader">Membros do Grupo</h5>${membersListHtml}</div><div class="content-block"><h5 class="subheader">Validação de Proteção de Dados</h5><div class="validation-grid"><div class="validation-item"><label>Política de Backup</label><div class="validation-value">${backupHtml}</div></div><div class="validation-item"><label>Replicação Cross-Region</label><div class="validation-value">${crossRegionHtml}</div></div><div class="validation-item"><label>Destino da Replicação</label><div class="validation-value">${validation.cross_region_target}</div></div></div></div>`;
    ...
    const vcnsHtml = vcns?.length > 0 ? vcns.map(vcn => {
      ...
      const lpgTable = createTable(['Nome', 'Status do Peering', 'Route Table', 'CIDR Anunciado', 'Cross-Tenancy'], vcn.lpgs?.map(lpg => {
        const statusClass = lpg.peering_status?.toLowerCase() || 'unknown';
        const statusText = lpg.peering_status_details || lpg.peering_status;
        return [`<span class="text-highlight">${lpg.display_name}</span>`, `<span class="status-badge status-${statusClass}">${statusText}</span>`, lpg.route_table_name, lpg.peer_advertised_cidr || 'N/A', lpg.is_cross_tenancy_peering ? 'Sim' : 'Não'];
      }));
    ...
    const loadBalancersHtml = load_balancers?.length > 0 ? load_balancers.map(lb => {
      ...
      const cardContent = `<fieldset><legend>Informações Gerais</legend><div class="grid-container"><div class="info-group full-width"><label>Endereços IP</label><div class="info-value"><ul class="clean-list">${lb.ip_addresses?.map(ip => `<li>${ip.ip_address} (${ip.is_public ? 'Público' : 'Privado'})</li>`).join('') || '<li>N/A</li>'}</ul></div></div><div class="info-group full-width"><label>Hostnames Virtuais</label>${createTable([], lb.hostnames?.map(h => [`<span class="text-highlight">${h.name}</span>`]))}</div></div></fieldset><hr class="fieldset-divider"><div class="content-block"><h5 class="subheader">Listeners</h5>${createTable(['Nome', 'Protocolo', 'Porta', 'Backend Set Padrão'], lb.listeners?.map(l => [`<span class="text-highlight">${l.name}</span>`, l.protocol, l.port, l.default_backend_set_name]))}</div><div class="content-block"><h5 class="subheader">Backend Sets</h5>${(lb.backend_sets && lb.backend_sets.length > 0) ? lb.backend_sets.map(bs => `<div class="backend-set-details"><h6 class="tunnel-subheader">Backend Set: <span class="text-highlight">${bs.name}</span> (Política: ${bs.policy})</h6><ul class="tunnel-basic-info"><li><strong>Health Check:</strong> ${bs.health_checker.protocol}:${bs.health_checker.port}</li><li><strong>URL Path:</strong> ${bs.health_checker.url_path || 'N/A'}</li></ul>${createTable(['Nome', 'IP', 'Porta', 'Peso'], bs.backends?.map(b => [`<span class="text-highlight">${b.name}</span>`, b.ip_address, b.port, b.weight]))}</div>`).join('') : '<p class="no-data-message">Nenhum Backend Set encontrado.</p>'}</div>`;
    ...
    const drgsHtml = drgs?.length > 0 ? drgs.map(drg => {
      ...
      const rpcsTable = createTable(['Nome', 'Status', 'Status do Peering'], drg.rpcs?.map(rpc => {
    ...
    const ipsecHtml = ipsec_connections?.length > 0 ? ipsec_connections.map(ipsec => {
      ...
      const tunnelsHtml = ipsec.tunnels.map(tunnel => {
        ...
        if (tunnel.routing_type === 'BGP' && tunnel.bgp_session_info) {
          ...
          bgpDetailsHtml = `<div class="crypto-details-grid"><div><h6 class="tunnel-subheader">Sessão BGP</h6><ul><li><strong>ASN Oracle:</strong> ${bgp.oracle_bgp_asn || 'N/A'}</li><li><strong>ASN Cliente:</strong> ${bgp.customer_bgp_asn || 'N/A'}</li></ul></div><div><h6 class="tunnel-subheader">IPs do Peering</h6><ul><li><strong>Interface Oracle:</strong> ${bgp.oracle_interface_ip || 'N/A'}</li><li><strong>Interface Cliente:</strong> ${bgp.customer_interface_ip || 'N/A'}</li></ul></div></div><hr class="tunnel-divider">`;
        }
        return `<div class="tunnel-details"><div class="tunnel-header"><strong>Túnel: <span class="text-highlight">${tunnel.display_name}</span></strong><span class="status-badge status-${tunnel.status.toLowerCase()}">${tunnel.status}</span></div><ul class="tunnel-basic-info"><li><strong>IP Oracle:</strong> ${tunnel.vpn_oracle_ip || 'N/A'}</li><li><strong>IP do CPE:</strong> ${tunnel.cpe_ip || 'N/A'}</li><li><strong>Roteamento:</strong> ${tunnel.routing_type}</li><li><strong>IKE:</strong> ${tunnel.ike_version}</li></ul>${bgpDetailsHtml}<div class="crypto-details-grid"><div><h6 class="tunnel-subheader">Fase 1 (IKE)</h6><ul><li><strong>Autenticação:</strong> ${p1.authentication_algorithm}</li><li><strong>Criptografia:</strong> ${p1.encryption_algorithm}</li><li><strong>Grupo DH:</strong> ${p1.dh_group}</li><li><strong>Lifetime:</strong> ${p1.lifetime_in_seconds}s</li></ul></div><div><h6 class="tunnel-subheader">Fase 2 (IPSec)</h6><ul><li><strong>Autenticação:</strong> ${p2.authentication_algorithm || 'N/A'}</li><li><strong>Criptografia:</strong> ${p2.encryption_algorithm}</li><li><strong>Lifetime:</strong> ${p2.lifetime_in_seconds}s</li></ul></div></div></div>`
      }).join('<hr class="tunnel-divider">');
      const hasBgpTunnel = ipsec.tunnels.some(t => t.routing_type === 'BGP');
      const routingDisplay = hasBgpTunnel ? `<span class="full-width"><strong>Roteamento:</strong> BGP</span>` : `<span class="full-width"><strong>Rotas Estáticas:</strong> ${(ipsec.static_routes && ipsec.static_routes.length > 0) ? ipsec.static_routes.join(', ') : 'Nenhuma'}</span>`;
    ...
    const okeClustersHtml = kubernetes_clusters?.length > 0 ? kubernetes_clusters.map(cluster => {
      ...
      return `<div class="instance-summary-card collapsible">
    ...
    }).join('') : '<p class="no-data-message">Nenhum Cluster Kubernetes encontrado.</p>';
    if (selectedDocType === 'kubernetes') {
      return `<div>
                  <h3 class="infra-summary-main-title">Resumo dos Clusters Kubernetes: ${selectedCompartmentName}</h3>
                  <fieldset>
                    <legend><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="legend-icon"><circle cx="12" cy="12" r="10"></circle><circle cx="12" cy="12" r="3"></circle><line x1="12" y1="2" x2="12" y2="5"></line><line x1="12" y1="19" x2="12" y2="22"></line><line x1="22" y1="12" x2="19" y2="12"></line><line x1="5" y1="12" x2="2" y2="12"></line><line x1="19.07" y1="4.93" x2="16.24" y2="7.76"></line><line x1="7.76" y1="16.24" x2="4.93" y2="19.07"></line><line x1="19.07" y1="19.07" x2="16.24" y2="16.24"></line><line x1="7.76" y1="7.76" x2="4.93" y2="4.93"></line></svg>Clusters OKE</legend>
                    <div class="oke-container">${okeClustersHtml}</div>
                  </fieldset>
              </div>`;
    }
    const fullInfraHtml = !isNewHostFlow ? `
        <hr class="fieldset-divider">
        <fieldset><legend><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="legend-icon"><path d="M22 12h-4l-3 9L9 3l-3 9H2"></path></svg>Virtual Cloud Networks (VCNs)</legend><div class="vcn-container">${vcnsHtml || '<p class="no-data-message">Nenhuma VCN encontrada.</p>'}</div></fieldset>
        <hr class="fieldset-divider">
        <fieldset><legend><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="legend-icon"><circle cx="12" cy="12" r="10"></circle><circle cx="12" cy="12" r="3"></circle><line x1="12" y1="2" x2="12" y2="5"></line><line x1="12" y1="19" x2="12" y2="22"></line><line x1="22" y1="12" x2="19" y2="12"></line><line x1="5" y1="12" x2="2" y2="12"></line><line x1="19.07" y1="4.93" x2="16.24" y2="7.76"></line><line x1="7.76" y1="16.24" x2="4.93" y2="19.07"></line><line x1="19.07" y1="19.07" x2="16.24" y2="16.24"></line><line x1="7.76" y1="7.76" x2="4.93" y2="4.93"></line></svg>Clusters Kubernetes (OKE)</legend><div class="oke-container">${okeClustersHtml}</div></fieldset>
        <hr class="fieldset-divider">
        <fieldset><legend><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="legend-icon"><path d="M12 21a9 9 0 0 0 9-9 9 9 0 0 0-9-9 9 9 0 0 0-9 9 9 9 0 0 0 9 9Z"></path><path d="M8 12a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1h1a1 1 0 0 0 1-1v-1a1 1 0 0 0-1-1Z"></path><path d="M15 12a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1h1a1 1 0 0 0 1-1v-1a1 1 0 0 0-1-1Z"></path><path d="M12 2v2"></path><path d="M12 8v2"></path></svg>Load Balancers (LBaaS)</legend><div class="lb-container">${loadBalancersHtml || '<p class="no-data-message">Nenhum Load Balancer encontrado.</p>'}</div></fieldset>
        <hr class="fieldset-divider">
        <fieldset><legend><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="legend-icon"><path d="M2 12h20M7 7l5 5-5 5M17 7l-5 5 5 5"></path></svg>Conectividade de Roteamento</legend><div class="drg-container">${drgsHtml || '<p class="no-data-message">Nenhum DRG encontrado.</p>'}</div></fieldset>
        <hr class="fieldset-divider">
        <fieldset><legend><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="legend-icon"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path></svg>Conectividade VPN</legend><h4 class="subheader">Customer-Premises Equipment (CPEs)</h4>${cpesHtml}<h4 class="subheader">Conexões VPN IPSec</h4><div class="ipsec-container">${ipsecHtml || '<p class="no-data-message">Nenhuma conexão IPSec encontrada.</p>'}</div></fieldset>
    ` : '';
  ...
  const generateDocument = () => {
    ...
    if (!allInfrastructureData || (!allInfrastructureData.instances?.length && !allInfrastructureData.kubernetes_clusters?.length)) {
    ...
    try {
      ...
      xhr.onload = function() {
        if (this.status === 200) {
          ...
          let compartmentName = "Generic";
          if (allInfrastructureData.instances.length > 0) {
            compartmentName = allInfrastructureData.instances[0].compartment_name;
          } else if (allInfrastructureData.kubernetes_clusters.length > 0) {
            compartmentName = selectedCompartmentName;
          }
          compartmentName = compartmentName.replace('SERVERS-', '').replace(/[^a-zA-Z0-9_-]/g, '');

          let docIdentifier = "Geral";
          if (selectedDocType === 'full_infra') docIdentifier = "Infraestrutura";
          else if (selectedDocType === 'new_host') docIdentifier = "NovoHost";
          else if (selectedDocType === 'kubernetes') docIdentifier = "Kubernetes";
          ...
          a.download = `Doc_${docIdentifier}_${compartmentName}_${timestamp}.docx`;
