...
class _ProgressProxy:
    """Scales inner per-compartment progress (0-100) into a sub-range of total progress."""
    ...
    def update_state(self, state, meta):
        if state == "PROGRESS" and meta:
            inner = meta.get("current", 0)
            scaled = self._base + int(inner / 100 * self._range)
...
def collect_full_infrastructure_task(self, region, compartments, doc_type, include_standalone=True, profile_id=None):
    """Collect full infrastructure data for one or more compartments.

    Args:
        compartments: list of {id, name} dicts  OR  a plain compartment_id string
                      (legacy callers that queued tasks before this change).
    """
    # Backwards compatibility: old callers passed compartment_id as a plain string.
