...
def _add_instances_table(document: Document, instances: List[InstanceData]):
    ...
    if not instances:
        return
    document.add_paragraph(
        "A tabela a seguir detalha as configurações das instâncias computacionais no escopo."
    )
    headers = [
        "HOST",
        "SHAPE",
...
def _add_volume_and_backup_section(
    ...
    _add_and_bookmark_heading(
        document, "Gerenciamento de Volumes e Backup", 2, toc_list, counters
    )
    _add_and_bookmark_heading(
        document, "Volumes Anexados (Block Volumes)", 3, toc_list, counters
    )
    all_block_volumes = [vol for data in instances for vol in data.block_volumes]
    if all_block_volumes:
        document.add_paragraph(
            "A tabela abaixo detalha os Block Volumes anexados às instâncias."
        )
        ...
        for data in instances:
            for vol in data.block_volumes:
                ...
                if vol.id in volume_to_vg_map:
                    cells[3].text = f"Gerenciado por VG: {volume_to_vg_map.get(vol.id)}"
                else:
    ...
    document.add_paragraph(
        "A tabela a seguir consolida as políticas de backup para todos os volumes (Boot e Block) das instâncias."
    )
    ...
    for instance in instances:
        ...
        if instance.boot_volume_id in volume_to_vg_map:
            boot_cells[2].text = (
                f"Gerenciado por VG: {volume_to_vg_map.get(instance.boot_volume_id)}"
            )
        else:
        ...
        for vol in instance.block_volumes:
            ...
            if vol.id in volume_to_vg_map:
                block_cells[2].text = f"Gerenciado por VG: {volume_to_vg_map.get(vol.id)}"
            else:
    ...
    if has_instance_policy or has_vg_policy:
        _add_and_bookmark_heading(
            document, "Detalhes da Política de Backup CCM-7D", 4, toc_list, counters
        )
        document.add_paragraph(
            "O Backup CCM-7D está configurado por meio de uma Backup Policy, garantindo a proteção contínua dos dados."
        )
        document.add_paragraph(
            "A política define um agendamento diário Incremental, executado à 01:00 (horário regional)."
        )
        document.add_paragraph("Os backups gerados possuem retenção de 7 dias.")
        document.add_paragraph(
            "Essa política garante que, em caso de falhas, seja possível recuperar o volume rapidamente."
        )
    document.add_paragraph()
...
def _add_volume_groups_section(
    ...
    document.add_paragraph(
        "Volume Groups são conjuntos de volumes (Boot e Block) que podem ser gerenciados como uma única unidade, especialmente para backups consistentes e replicação entre regiões."
    )
    for vg in sorted(infra_data.volume_groups, key=lambda v: v.display_name):
        _add_and_bookmark_heading(
            document, f"Volume Group: {vg.display_name}", 3, toc_list, counters
        )
        info_data = {
            "Nome": vg.display_name,
            "Estado": vg.lifecycle_state,
            "Availability Domain": vg.availability_domain,
            "Total de Membros": str(len(vg.members)),
        }
        ...
        val = vg.validation
        validation_data = {
            "Política de Backup": val.policy_name,
            "Replicação Cross-Region": "Habilitada"
            if val.is_cross_region_replication_enabled
            else "Desabilitada",
            "Destino da Replicação": val.cross_region_target,
        }
        _create_titled_key_value_table(
            document, "Validação de Proteção de Dados", validation_data
        )
...
def _add_vcn_details_section(
    ...
    _add_and_bookmark_heading(
        document, "Topologia de Rede Virtual (VCN)", 2, toc_list, counters
    )
    if not infra_data.vcns:
        document.add_paragraph("Nenhuma VCN encontrada neste escopo.")
        return
    for vcn in infra_data.vcns:
        ...
        _add_and_bookmark_heading(
            document, "Network Security Groups (NSGs)", 4, toc_list, counters
        )
        if vcn.network_security_groups:
            ...
            document.add_paragraph(
                "Nenhum Network Security Group encontrado para esta VCN."
            )

        _add_and_bookmark_heading(
            document, "Local Peering Gateways (LPGs)", 4, toc_list, counters
        )
        if vcn.lpgs:
            headers = [
                "Nome",
...
def _add_kubernetes_section(
    ...
    if not hasattr(infra_data, "kubernetes_clusters") or not infra_data.kubernetes_clusters:
        return

    _add_and_bookmark_heading(
        document, "CONFIGURAÇÃO AMBIENTE OKE", 1, toc_list, counters
    )
    for cluster in sorted(infra_data.kubernetes_clusters, key=lambda c: c.name):
        _add_and_bookmark_heading(
            document, f"Cluster: {cluster.name}", 2, toc_list, counters
        )
        headers = [
            "NOME DO CLUSTER",
            "VERSÃO KUBERNETES",
        ...
        _add_and_bookmark_heading(
            document, "Node Pools (Worker Nodes)", 3, toc_list, counters
        )
        if cluster.node_pools:
            ...
            document.add_paragraph("Nenhum Node Pool encontrado para este cluster.")
...
def _add_load_balancers_section(
    ...
    for lb in infra_data.load_balancers:
        _add_and_bookmark_heading(
            document, f"Load Balancer: {lb.display_name}", 3, toc_list, counters
        )
        ip_strings = [
            f"{ip.ip_address} ({'Público' if ip.is_public else 'Privado'})"
            for ip in lb.ip_addresses
        ]
        ip_list = "\n".join(ip_strings)
        lb_info = {
            "Nome": lb.display_name,
            "Shape": lb.shape_name,
            "Estado": lb.lifecycle_state,
            "Endereços IP": ip_list,
        }
        _create_titled_key_value_table(
            document, "Informações Gerais do Load Balancer", lb_info
        )
        ...
        if lb.backend_sets:
            for bs in lb.backend_sets:
                bs_info = {
                    "Política de Balanceamento": bs.policy,
                    "Health Checker": f"{bs.health_checker.protocol}:{bs.health_checker.port} (Path: {bs.health_checker.url_path})",
                }
                _create_titled_key_value_table(
                    document, f"Configuração do Backend Set: {bs.name}", bs_info
                )
                if bs.backends:
...
def _add_connectivity_section(
    ...
    _add_and_bookmark_heading(
        document, "Conectividade Externa e Roteamento", 2, toc_list, counters
    )
    _add_and_bookmark_heading(
        document, "Dynamic Routing Gateways (DRGs)", 3, toc_list, counters
    )
    if not infra_data.drgs:
        ...
        for drg in infra_data.drgs:
            _add_and_bookmark_heading(
                document, f"DRG: {drg.display_name}", 4, toc_list, counters
            )
            ...
            _add_and_bookmark_heading(
                document, "Conexões de Peering Remoto (RPCs)", 5, toc_list, counters
            )
            if drg.rpcs:
                ...
                document.add_paragraph(
                    "Nenhuma Conexão de Peering Remoto (RPC) associada a este DRG."
                )

    _add_and_bookmark_heading(
        document, "Customer-Premises Equipment (CPEs)", 3, toc_list, counters
    )
    ...
    if not infra_data.ipsec_connections:
        ...
        _add_and_bookmark_heading(
            document, "Sumário de Conexões VPN", 4, toc_list, counters
        )
        ...
        document.add_paragraph()
        _add_and_bookmark_heading(
            document, "Detalhes das Conexões VPN", 4, toc_list, counters
        )
        for ipsec in infra_data.ipsec_connections:
            document.add_paragraph(
                f"Configuração da Conexão: {ipsec.display_name}", style="Heading 5"
            )
            ...
            for tunnel in ipsec.tunnels:
                p = document.add_paragraph()
                p.add_run(f"Túnel: {tunnel.display_name} ").bold = True
                p.add_run(f"(Status: {tunnel.status})")
                tunnel_info = {
                    "IP Oracle": tunnel.vpn_oracle_ip or "N/A",
                    "IP do CPE": tunnel.cpe_ip or "N/A",
                    "Roteamento": tunnel.routing_type,
                    "Versão IKE": tunnel.ike_version,
                }
                _create_titled_key_value_table(
                    document, "Informações do Túnel", tunnel_info
                )
                if tunnel.routing_type == "BGP" and tunnel.bgp_session_info:
                    ...
                    _create_titled_key_value_table(
                        document, "Detalhes da Sessão BGP", bgp_info_data
                    )
                p1 = tunnel.phase_one_details
                p1_info = {
                    "Autenticação": p1.authentication_algorithm,
                    "Criptografia": p1.encryption_algorithm,
                    "Grupo DH": p1.dh_group,
                    "Lifetime (s)": str(p1.lifetime_in_seconds),
                }
                _create_titled_key_value_table(document, "Fase 1 (IKE)", p1_info)
                p2 = tunnel.phase_two_details
                p2_info = {
                    "Autenticação": p2.authentication_algorithm or "N/A",
                    "Criptografia": p2.encryption_algorithm,
                    "Lifetime (s)": str(p2.lifetime_in_seconds),
                }
                ...
                if (
                    ...
                    p.add_run(
                        "Os parâmetros de criptografia customizados para este túnel divergem das recomendações padrão da Oracle."
                    )
...
def generate_documentation(
    ...
    doc_type_map = {
        "full_infra": ("Documentação de Infraestrutura", "Infraestrutura"),
        "new_host": ("Documentação de Novo Host", "NovoHost"),
        "kubernetes": ("Documentação de Kubernetes (OKE)", "Kubernetes"),
    }
    doc_title_text, doc_identifier = doc_type_map.get(
        doc_type, ("Documentação Técnica", "Geral")
    )
    ...
    if architecture_image_bytes_list:
        _add_and_bookmark_heading(
            document, "Arquitetura e Escopo", 1, headings_for_toc, numbering_counters
        )
        _add_and_bookmark_heading(
            document, "Desenho da Arquitetura", 2, headings_for_toc, numbering_counters
        )
    ...
    if doc_type == "kubernetes":
        ...
        _add_kubernetes_section(
            document, filtered_infra_data, headings_for_toc, numbering_counters
        )
        ...
        _add_volume_and_backup_section(
            document, infra_data, headings_for_toc, numbering_counters
        )
        if hasattr(infra_data, "volume_groups") and infra_data.volume_groups:
        ...
        _add_and_bookmark_heading(
            document,
            "Conectividade de Rede da(s) Instância(s)",
            2,
            headings_for_toc,
            numbering_counters,
        )
        ...
        for data in infra_data.instances:
            ...
            if data.route_table:
                entry = rt_map.setdefault(
                    data.route_table.name, {"rules": data.route_table.rules, "hosts": []}
                )
                if data.host_name not in entry["hosts"]:
                    entry["hosts"].append(data.host_name)
        _add_and_bookmark_heading(
            document, "Security Lists", 3, headings_for_toc, numbering_counters
        )
        for name, info in sorted(sl_map.items()):
            _add_network_resource_details(
                document, "Regras da Security List", name, info["rules"], info.get("hosts"), "security"
            )
        _add_and_bookmark_heading(
            document, "Network Security Groups", 3, headings_for_toc, numbering_counters
        )
        for name, info in sorted(nsg_map.items()):
            _add_network_resource_details(
                document, "Network Security Group", name, info["rules"], info.get("hosts"), "security"
            )
        _add_and_bookmark_heading(
            document, "Route Tables", 3, headings_for_toc, numbering_counters
        )
        for name, info in sorted(rt_map.items()):
            _add_network_resource_details(
                document, "Regras da Route Table", name, info["rules"], info.get("hosts"), "route"
            )
    else:  # 'full_infra'
        _add_and_bookmark_heading(
            document,
    ...
    _add_responsible_section(
        document, headings_for_toc, numbering_counters, responsible_name
    )
