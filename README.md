...
- **Multi-Language Support (i18n)**: The interface, progress updates, and the final `.docx` document are fully translatable (Portuguese/English), with instant language switching on the frontend.
...
- **i18n**: JSON-based internationalization for dynamic translation.
...
│   ├── doc_generator.py     # Logic for .docx document generation (i18n)
│   ├── generated_docs/      # Directory where documents are saved
│   ├── main.py              # FastAPI API (endpoints)
│   ├── oci_connector.py     # OCI integration logic (i18n progress)
...
└── frontend/
    ...
    │   └── style.css
    ├── js/
    │   └── app.js           # Main frontend logic (i18n handler)
    ├── locales/
...
- **Access the Frontend:** Open `frontend/index.html` in your web browser.
...
### 2. OCI IAM Policy Setup (Dynamic Group)

The application runs on an OCI VM using **Instance Principal** for authentication.  
This requires creating a **Dynamic Group** and **Policies** to grant read-only access.

#### 2a. Create Dynamic Group
Go to: **Identity & Security → Dynamic Groups**

Create a group (e.g. `oci-docgen-dg`) and use a matching rule:
```
...
For more details on OCI IAM policy verbs and resource types, refer to the official Oracle documentation:  
[Oracle Cloud Infrastructure IAM Policy Reference](https://docs.oracle.com/pt-br/iaas/Content/Identity/Reference/corepolicyreference.htm)
...
### 5. systemd Service for Gunicorn API
...
### 6. systemd Service for Celery Worker
